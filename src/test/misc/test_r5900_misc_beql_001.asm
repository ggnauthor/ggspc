...
;*********** YOUR CODE HERE **************
  ...
  beql s0, s1, label1  ;ジャンプしない
  addi t0, zero, 1  ;実行されない命令
  addi t1, zero, 1
  j label2
  nop
label1:
  addi t2, zero, 1  ;実行されない命令
label2:
  beql s0, s2, label3  ;ジャンプする
  addi t3, zero, 1
  addi t4, zero, 1  ;実行されない命令
label3:
  beql s0, s3, label4  ;ジャンプしない
  addi t5, zero, 1  ;実行されない命令
  addi t6, zero, 1
label4:
...
loop:
  nop
  beql at, a0, loop
  addi at, at, 1    ;1回実行(抜ける時は無効にされるため)
