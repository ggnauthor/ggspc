...
;*********** YOUR CODE HERE **************
  ...
  ; ノーマル(符号拡張)
  lh a0,  2(v0)  ; 符号なしオフセット
  lh a1, -2(v0)  ; 符号付きオフセット
  lh a2,  0(v0)  ; オフセットなし
  
  ; アドレス変換(符号拡張)
  lh s0,  2(v1)  ; 符号なしオフセット
  lh s1, -2(v1)  ; 符号付きオフセット
  lh s2,  0(v1)  ; オフセットなし
  
  ; ノーマル(ゼロ拡張)
  lhu t0,  2(v0)  ; 符号なしオフセット
  lhu t1, -2(v0)  ; 符号付きオフセット
  lhu t2,  0(v0)  ; オフセットなし
  
  ; アドレス変換(ゼロ拡張)
  lhu t3,  2(v1)  ; 符号なしオフセット
  lhu t4, -2(v1)  ; 符号付きオフセット
  lhu t5,  0(v1)  ; オフセットなし
