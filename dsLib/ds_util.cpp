...
void ds_util::getParentDirectory(const char* p_path, char* p_out) {
  ...
  if (p1 > p2) {
    strncpy(p_out, p_path, p1 - p_path);
    p_out[p1 - p_path] = '\0';
  } else if (p2 > p1) {
    strncpy(p_out, p_path, p2 - p_path);
    p_out[p2 - p_path] = '\0';
  } else {
    strcpy(p_out, p_path);
  }
...
void ds_util::getRelativePath(const char* p_path1, const char* p_path2, char* p_out, bool p_matchcase)
{
  ...
  {
    if (tmp1[i] == '\\') tmp1[i] = '/';
    else if (p_matchcase == false) tmp1[i] = tolower(tmp1[i]);
  }
  for (int i = 0; i < len2; i++)
  {
    if (tmp2[i] == '\\') tmp2[i] = '/';
    else if (p_matchcase == false) tmp2[i] = tolower(tmp2[i]);
  }
...
std::string ds_util::getFullPath(const std::string& p_path1,
                                 const std::string& p_path2,
                                 bool p_matchcase) {
  char dir[256];
  getParentDirectory(p_path1.c_str(), dir);
  std::string path = std::string(dir) + "/" + p_path2;

  // パスの正規化
  for (int i = 0; i < (int)path.size(); i++) {
    if (path[i] == '\\') path[i] = '/';
    else if (p_matchcase == false) path[i] = tolower(path[i]);
  }
  std::vector<std::string> list;
  ds_util::split_string(path, &list, "/", true);
  ...
  return join_string(list, '/', '\0', '\0');
}

// -----------------------------------------------------------------------------
void ds_util::split_string(const std::string& p_in,
                  std::vector<std::string>* p_out,
                  std::string p_delimiter, bool p_shrink) {
  std::string::size_type last_pos = 0, pos = 0;
  p_out->clear();
  do {
    pos = p_in.find(p_delimiter, last_pos);
    int len = pos - last_pos;
    if (pos == std::string::npos) len = p_in.length() - last_pos;

    std::string tmp = boost::algorithm::trim_copy(p_in.substr(last_pos, len));
    if (!p_shrink || tmp.length() > 0) {
      p_out->push_back(tmp);
    }
    last_pos = pos + p_delimiter.size();
  } while (pos != std::string::npos);
}

// -----------------------------------------------------------------------------
std::string ds_util::join_string(const std::vector<std::string>& p_in,
                        char p_delimiter, char p_wrap1, char p_wrap2) {
  std::ostringstream oss;
  for (int i = 0; i < (int)p_in.size(); i++) {
    if (p_wrap1 != '\0') oss << p_wrap1;
    oss << p_in[i];
    if (p_wrap2 != '\0') oss << p_wrap2;
    if (p_delimiter != '\0' && i < (int)p_in.size() - 1) oss << p_delimiter;
  }
  return oss.str();
}
